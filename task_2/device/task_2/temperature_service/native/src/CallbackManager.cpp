...
void CallbackManager::unregisterCallback(const Callback &cb)
{
    ...
    mCallbacks.erase(
        std::remove_if(mCallbacks.begin(), mCallbacks.end(),
                       [&](const std::weak_ptr<aidl::temperature::service::api::IReadingsCallback> &w)
                       {
                           auto s = w.lock();
                           return (!s) || (s == cb);
                       }),
...
void CallbackManager::notifyAll(const std::vector<aidl::temperature::service::api::TemperatureReading> &readings)
{
